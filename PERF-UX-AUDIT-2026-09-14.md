...
2. Run `node tools/perf/baseline.mjs` **before** touching anything and save the output.
...
6. Tick the item's checkbox in the "Progress" table at the bottom of this file and commit that too.
...
`.github/workflows/deploy.yml`, in the `Stage the servable site` step, add to the exclude list:
...
Push, then check the Actions log line `staged NNNMB` — it must drop by ~9 MB.
Then confirm `dataset/questions.csv` still exists in the artifact.
